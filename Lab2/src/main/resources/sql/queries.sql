...
where p.function_id = (select t.id from labs.public.functions t where t.function_type = 'tips')
  and p.y = 50.0
...
from labs.public.points p,
     labs.public.functions f
where p.function_id = (select t.id from labs.public.functions t where t.function_type = 'tip')
  and p.y = 1.0
