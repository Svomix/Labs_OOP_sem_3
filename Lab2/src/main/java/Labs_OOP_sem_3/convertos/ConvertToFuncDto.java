...
public class ConvertToFuncDto {
    public static FunctionDtoList convert(FunctionEntity function) {
        FunctionDtoList functionDtoList = FunctionDtoList.builder().hash(function.getHash()).id_user((long) function.getId_user()).type(function.getType()).id(function.getId()).build();
